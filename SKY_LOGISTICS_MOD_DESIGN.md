...
- `天穹玻璃`：使用 1 个已充能天穹水晶 + 8 个 `#forge:glass` 合成，是全亮度光源玻璃方块；保留完整一阶结构后，在外圈四角上方额外添加，用于将供奉结构提升到二阶。
...
- 一阶结构使用完整天穹石外圈；二阶结构不替换一阶方块，只要求在 5x5 外圈四个角的上方额外添加天穹玻璃。
...
- Patchouli 数据提供一本 `天穹物流手册` 和一个上述二阶多方块预览。JEI 页面作为可选 compat 源码存在，默认无 JEI API jar 时不参与基础构建。
...
- 线路 ID 是持久化 UUID，工具和 GUI 只显示短码，例如 `A7F3-2C`。
...
线路 D812-4A：流体供应
线路 F0E1-99：主电网
线路 B91D-08：另一个基地或另一个系统
```

玩家通过“线路短码”识别线路；工具图标、节点 GUI 和 Jade tooltip 显示当前线路短码。

线路 ID 的生成和复制：

- 新工具第一次打开 GUI 时可以生成一条新线路 ID。
- 对已有节点使用复制模式时，工具复制该节点的线路 ID 和配置模板。
- 工具 GUI 可手动粘贴/输入线路短码或完整 ID，用于多人共享同一线路。
- 线路短码只用于显示；内部以完整 UUID 区分，避免短码碰撞。
...
- 工具 GUI 中设置当前线路 ID、放置模板的资源类型、红石条件和优先级；抽取/存入不在配置器里切换，而由普通放置/潜行放置决定。
- 工具图标或 tooltip 显示当前线路短码。
- 潜行滚轮：切换复制/粘贴模式，或作为后续快捷操作预留。
- 潜行右键节点：复制该节点的线路 ID，以及六个面的模式、资源类型、红石条件和优先级。
- 粘贴模式右键节点：把复制到工具中的六面配置粘贴到该节点。
- 过滤升级不塞进配置器复制数据；过滤规则由节点每面的过滤升级槽承载，避免工具 NBT 过重，也避免复制时意外复用复杂过滤列表。
...
副手持空域配置器
主手放置空域节点
新节点自动读取副手工具的线路 ID，以及工具 GUI 中配置的本面放置模板
普通放置为存入，潜行放置为抽取
节点直接链接到对应线路
```
...
```text
1. 打开工具 GUI，生成或选择线路 A7F3-2C。
2. 工具模式设为“输入源”，资源类型勾选物品 + 流体 + FE。
3. 副手拿工具，主手拿节点，对机器输出侧放置。
4. 新节点自动成为 A7F3-2C 的输入源，可同时抽物品、流体和能量。
5. 切工具为“输出目标”，在另一个容器/机器侧面放置节点。
6. 两端线路 ID 相同，线路自动工作。
```
...
- 不作为配置器复制数据的一部分；需要复用复杂过滤时，玩家直接复用同一个配置好的过滤升级物品。
- 使用幽灵槽，不真实消耗或存储样品物品。
...
- 匹配模式：
  - 精确物品。
  - 标签匹配，例如 `#forge:ingots/iron`。
  - 模组 ID 匹配，例如 `minecraft:*`、`thermal:*`。
  - NBT/组件精确匹配。
  - 可选忽略耐久、忽略组件、忽略数量。
  - 流体样品和流体标签，MVP 可先做基础流体匹配。
...
- `天穹玻璃`：1 个已充能天穹水晶 + 8 个 `#forge:glass` 合成，是全亮度光源方块；保留供奉法阵一阶外圈后，加在四个外圈角上方时构成二阶祭坛。
...
- `空域配置器`、`天穹过滤列表`、速度/跨维升级：进入常规工作台合成路线，部分升级消耗星辉甘露。
...
- 星辉甘露供奉配方。
- 祭坛/供桌/天穹石/天穹玻璃组成的 5x5 供奉多方块结构，支持一阶/二阶判断。
- Patchouli 手册数据和一个多方块预览条目。
- 复制粘贴配置。
- 失败 backoff。
- 存档重载后的普通端点重绑。
