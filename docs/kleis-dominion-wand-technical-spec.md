...
- 目标是天穹分配器时，不透视端点面本身；改为在分配器内部 `[0.25, 0.75]³` 绘制 alpha `0.50` 的中心立方体，并使用 `GREATER` 深度测试、只写颜色的遮挡通道显示被实体方块遮住的片段。此标记方式与 AE2LT 的 see-through inner cube 一致。其它目标不绘制中心立方体或遮挡通道。
- 关闭面剔除或为六个方向生成正确顶点顺序。
- 半透明动画层不写深度。
