...
覆盖层要求主手持有克莱斯支配之杖，且副手持有已绑定线路的 `ConfiguratorItem`。
...
- `KleisDominionWandOverlayRequestPacket`
  - 客户端持杖且副手配置器有线路时请求当前维度、当前线路附近端点
  - 服务端从副手配置器重新读取 lineId，不能信任客户端声明
...
主手首次切换到克莱斯支配之杖，且副手配置器提供有效 `lineId` 时立即请求该线路快照；之后：

- 玩家跨越一定距离或进入新区块时刷新。
- 副手配置器被移除、换手或失去线路信息时立即清空缓存并停止请求。
- 配置器切换当前线路时立即清空旧缓存并请求新 `lineId`。
- 默认最多每 20 tick 请求一次。
- 登录、退出、切维度时清空缓存。
...
4. 没有副手配置器或配置器没有当前线路时不显示覆盖层。
5. 覆盖层只显示副手配置器当前 `lineId` 的端点。
