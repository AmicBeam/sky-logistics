...
- 使用该端点所属线路再次右击已绑定侧面：直接删除端点；选择其它线路时拒绝操作并提示线路冲突。
- 左击已绑定侧面：打开单端点节点 GUI。
- 左击方块时绝不进入方块破坏流程。
- 左击实体时取消攻击伤害，并尝试把该实体传送到同维度 `Y=256`。
- 主手持杖且副手配置器提供当前线路时，只显示该线路的端点覆盖层。
- 主手持配置器、副手持杖时进入“覆盖面编辑模式”：显示附近有权查看的虚拟端点，可潜行右击复制端点配置，用普通右击把配置器中的复制配置粘贴到端点，并可左击打开端点 GUI。
...
- 物品属性：史诗稀有度、最大堆叠数 1、`fireResistant`。
- 支配之杖、柯拉甘露、柯拉甘露块、天穹物品库和天穹流体库属于 `skylogistics:protected_drops` 标签；其掉落实体设置无限寿命和无敌，不会自然消失，也不会被爆炸、仙人掌、火焰、熔岩等伤害摧毁，但仍可正常拾取或被漏斗收集。
...
- 主手有配置器、副手有杖：进入第 5.3 节的覆盖面编辑模式，不触发杖的创建、删除或实体传送；命中已有覆盖面时左击会阻止破坏并打开端点 GUI。
...
- 左击已有虚拟端点覆盖面：打开该端点 GUI，不产生方块破坏进度。
...
右击手势命中已绑定的 `EndpointKey` 时，服务端先比较副手配置器当前 `lineId` 与端点所属线路：

1. `lineId` 不一致时拒绝绑定、删除或覆盖，保留原端点，并在 action bar 显示线路冲突提示。
2. `lineId` 一致时不区分普通右击或潜行右击，直接删除且不显示二次确认。
...
- `KleisDominionWandToggleEndpointPacket`
  - 右击未绑定面时创建；同线路右击已绑定面时删除；其它线路右击时返回线路冲突
...
- 自定义透视 `RenderType`／`RenderPipeline` 必须在第一次实际世界渲染时懒加载，不得在 NeoForge 模组扫描或客户端事件订阅类初始化阶段创建，避免渲染线程与 mod-loading worker 之间的启动等待。
- 关闭面剔除或为六个方向生成正确顶点顺序。
- 半透明动画层不写深度。
...
- 玩家仍在服务器上。
- 玩家当前维度等于请求维度。
- 创建、删除和实体传送请求中，主手仍是克莱斯支配之杖；需要端点配置时副手仍是配置器。
- 覆盖面复制／粘贴／右击打开／左击打开请求中，主手仍是配置器、副手仍是克莱斯支配之杖；主手杖左击打开时仍要求副手配置器。
...
- 同线路右击已绑定面只删除、不在同一次点击重建；其它线路右击不改变原端点并返回线路冲突
- 从配置器复制 placement 配置
...
最低祭坛等级：3
祭坛中央：32 × 已充能尤洛伽水晶
供桌材料：1 × 柯拉甘露块、1 × 维度升级卡、2 × 末地烛、16 × 天穹玻璃
结果：1 × 克莱斯支配之杖
```

祭坛中央的 32 个尤洛伽水晶必须全部已充能，在数据上写作 `charged: true`。柯拉甘露块作为供桌材料提交，不计入三阶祭坛结构本身所需的四个柯拉甘露块。
...
```json
{
  ...
  "offerings": [
    {
      "ingredient": {
        "item": "skylogistics:chora_nectar_block"
      },
      "count": 1
    },
    {
    ...
    {
      "ingredient": {
        "item": "skylogistics:celestial_glass"
      },
      "count": 16
    }
  ],
...
32 个尤洛伽水晶必须全部已充能：主材料条目写 `charged: true`。它们作为一组放在祭坛中央提交。
...
| 材料 | 数量 | 意象／成本 |
| --- | ---: | --- |
| 柯拉甘露块 | 1 | 作为承载虚拟节点的容受介质 |
| 维度升级卡 | 1 | 固化内置跨维度能力 |
| 末地烛 | 2 | 提供杖端的空间锚定 |
| 天穹玻璃 | 16 | 汇聚并稳定天穹能量 |

祭坛中央另需 32 个已充能尤洛伽水晶，为仪式提供天穹结晶能量。
...
世界观含义：中央 32 个已充能尤洛伽水晶提供天穹结晶能量；柯拉甘露块作为承载虚拟节点的“容受介质”；两根末地烛提供空间锚定；16 个天穹玻璃汇聚并稳定能量；唯一一张维度升级卡把跨维度能力固化在杖中。

### 29.4 配方验收

- 一阶／二阶祭坛不能启动配方。
- 中央少于 32 个尤洛伽水晶或物品不匹配时不能启动。
- 供桌材料必须精确满足 1 个柯拉甘露块、1 张维度升级卡、2 根末地烛和 16 个天穹玻璃。
- 中央的 32 个尤洛伽水晶必须全部已充能；未充能水晶不能启动配方。
...
- 三个版本全部实现并构建通过。
- 右击／潜行右击正确创建单面虚拟节点。
- 使用端点所属线路再次右击已绑定面直接删除端点，且不要求二次确认；其它线路右击保持原绑定并提示冲突。
- 左击打开单端点 GUI 且绝不破坏方块。
