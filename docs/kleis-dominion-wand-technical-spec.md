...
| 材料 | 建议数量 | 意象／成本 |
| --- | ---: | --- |
| 维度升级卡 | 1 | 已确认；赋予内置跨维度能力 |
| 已充能尤洛伽水晶 | 1 | 赋予并驱动“钥”的天穹权能 |
| 天穹玻璃 | 4 | 形成贴附在设备侧面的无形边界 |
| 末影之眼 | 4 | 定位远端设备与其它维度 |

建议基础 `duration = 960 tick`。在默认三阶 `4×` 工作倍率下，实际耗时约为 `240 tick = 12 秒`；服务器调整三阶倍率后，实际耗时按现有祭坛逻辑变化。

维度升级卡数量已冻结为 1 张。其余三种材料与数量仍是推荐候选，实施前需要最终确认。

这套组合的世界观含义：中央柯拉甘露作为承载虚拟节点的“容受介质”；已充能尤洛伽水晶提供天穹祝福与能量；天穹玻璃把无形接口限定在设备侧面；末影之眼负责跨距离与跨维度定位；维度升级卡把这些能力固化为可工作的物流端点。
...
10. 其它三种供物是否采用 1 枚已充能尤洛伽水晶、4 块天穹玻璃和 4 个末影之眼。
11. 配方基础时长是否采用 960 tick，使默认 4× 三阶祭坛实际耗时约 12 秒。
