...
3. 服务端按第 8.3 节将单面投影应用到目标端点，更新 SavedData、revision、运行时索引和 overlay delta。
...
- 粘贴会复制线路、单面模式、资源设置、红石、优先级、限制、过滤器和兼容升级。
- 粘贴可以改变目标端点的 `lineId`；必须先从旧线路索引注销，再注册到新线路，并分别向旧／新订阅者发送 delta。
...
- `EDIT_NEARBY` 下配置器切换线路不必重新请求全量端点，只更新当前线路高亮；复制或粘贴导致端点换线时由 delta 更新。
- 默认最多每 20 tick 请求一次。
- 登录、退出、切维度时清空缓存。
...
外框初始实现使用 `alpha = 0.85`。后续只允许基于游戏内可读性做小幅调整，不改变常驻语义。
...
- 动画框常规 `alpha = 0.50`
- 只允许在最后一个尺寸的 5 tick 内做 alpha 淡出；位置仍保持离散
...
- 准星射线命中某个已同步覆盖面时，该面恢复正常 alpha，并在准星附近显示“线路名 · 抽取／存入”。
- 配置器处于 `PasteMode` 时，命中目标额外显示“右击粘贴”；未处于粘贴模式时显示“潜行右击复制 · 右击配置”。
...
- 粘贴改变 lineId 时旧／新线路索引与 overlay delta 同步正确
...
64 个尤洛伽水晶不要求已充能：配方条目不写 `charged: true`。它们可以在一个供桌槽中以完整一组提交。
...
| 材料 | 数量 | 意象／成本 |
| --- | ---: | --- |
| 尤洛伽水晶 | 64 | 以完整一组天穹结晶提供仪式能量；不要求充能 |
| 维度升级卡 | 1 | 固化内置跨维度能力 |
| 回响碎片 | 8 | 为无形节点提供位置回响 |
...
- 尤洛伽水晶无论是否充能都按普通 ingredient 规则匹配；实现不得额外要求已充能。
...
3. 世界覆盖动画采用 50 tick／2.5 秒周期，外框 alpha 0.85、动画框 alpha 0.50。
