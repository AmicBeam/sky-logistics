...
public class SkyOfferingCategory implements IRecipeCategory<OfferingRecipe> {
    ...
    public void draw(OfferingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX,
        ...
        guiGraphics.drawString(font, Component.literal(recipe.duration() + " TICK"), 9, 9,
                0xFFEAFBFF, false);
        Component tierLabel = Component.literal("T" + recipe.requiredTier());
        guiGraphics.drawString(font, tierLabel, 102 - font.width(tierLabel), 9,
                0xFFEAFBFF, false);
    }
    ...
    private static void drawRitual(GuiGraphics graphics, int offeringCount) {
    ...
    private static void drawOfferingSlot(GuiGraphics graphics, int x, int y) {
        drawInputSlot(graphics, x, y);
    }

    private static void drawInputSlot(GuiGraphics graphics, int x, int y) {
        drawSlot(graphics, x, y, true, INPUT_BORDER, false);
    }

    private static void drawSlot(GuiGraphics graphics, int x, int y, boolean active, int accent) {
        drawSlot(graphics, x, y, active, accent, true);
    }

    private static void drawSlot(GuiGraphics graphics, int x, int y, boolean active, int accent, boolean glow) {
    ...
    private static void drawFrame(GuiGraphics graphics, int x, int y, int width, int height, int shadow, int light) {
    ...
    private static void drawGlowLineH(GuiGraphics graphics, int x1, int x2, int y, int color) {
        graphics.fill(x1, y - 2, x2, y + 3, 0x229EE5EE);
        graphics.fill(x1, y - 1, x2, y + 2, 0x669EE5EE);
        graphics.fill(x1, y, x2, y + 1, color);
    }

    private static void drawGlowLineV(GuiGraphics graphics, int x, int y1, int y2, int color) {
        graphics.fill(x - 2, y1, x + 3, y2, 0x229EE5EE);
        graphics.fill(x - 1, y1, x + 2, y2, 0x669EE5EE);
        graphics.fill(x, y1, x + 1, y2, color);
    }

    private static void drawConnection(GuiGraphics graphics, int centerX, int centerY, int targetX, int targetY) {
        if (targetX == centerX) {
    ...
    private static void drawRunes(GuiGraphics graphics) {
    ...
    private static void drawRune(GuiGraphics graphics, int x, int y) {
