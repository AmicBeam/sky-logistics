...
祭坛必须放在足够高的位置，并搭好有效供奉法阵。默认需要达到 Y=96，服务器设置可能有所不同。把主材料放在祭坛上，把供品放在四侧供桌上；供桌顺序不限。

材料齐全后，祭坛会出现粒子并开始计时。等待配方时间结束后，输入会一次性消失，产物会优先留在祭坛槽里；如果放不下，剩余产物会弹出到祭坛上方。

三阶祭坛按 `tierThreeAltarWorkSpeedMultiplier` 推进工作，默认 4 倍；服务器可在 1–64 倍之间调整。

如果装了 Jade，准星对准祭坛或供桌时可以看到其中的供品；空的供奉槽不会额外显示。
