...
The <ItemLink id="filter_list" /> stores exact item, fluid, and supported chemical samples in 18 ghost slots. It can use a whitelist or blacklist and can choose whether extra data and durability must match. Placing a list into a node face filter slot, or right-clicking a simple-pipe endpoint with it, copies the rules without consuming the item. Later edits to the original do not update copied rules.

Use <ItemLink id="tag_filter_list" /> when a whole item or fluid tag should match. It can also filter by source mod. Extracting items from an external network requires a whitelist containing concrete items.
