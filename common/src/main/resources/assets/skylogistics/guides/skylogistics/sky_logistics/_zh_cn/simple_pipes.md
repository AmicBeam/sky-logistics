...
对准物品或流体管道的机器端点，手持天穹过滤列表或标签过滤列表右击，可以把过滤规则复制到该端点，不会消耗列表。抽取和存入都会使用过滤，之后编辑原列表不会自动更新已复制规则。空手潜行右击可清除过滤；能量管道不接受过滤。安装 Jade 后可以查看端点状态和过滤摘要。
...
同一条线路通常最多包含 1024 个管道方块。放置或重新连接会超过上限时，新连接会保持断开并显示提示。
