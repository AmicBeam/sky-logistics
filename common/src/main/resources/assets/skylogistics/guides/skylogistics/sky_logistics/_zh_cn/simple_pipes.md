...
- Botania mana：每个抽取端每刻最多 50 mana，可用于给支持的魔力设备供能。
- Ars Nouveau 魔源：每个抽取端每刻最多 50 Source，可用于给支持的魔源设备供能。

同一条线路默认最多包含 1024 个管道方块。放置或重新连接会超过上限时，新连接会保持断开并显示提示。服务器可调整线路上限，也可关闭连接数量上限检查。
