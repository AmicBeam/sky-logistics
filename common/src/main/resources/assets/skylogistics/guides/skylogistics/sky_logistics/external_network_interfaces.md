...
Extracting items from an external network requires a whitelist containing at least one concrete item. Add the items you want to extract to that whitelist; tag or mod rules alone are not enough. Disabling an integration switch does not remove interface blocks or damage saved lines.

Warden Soul filters use Industrial Foregoing: Souls items as samples. A whitelist containing such an item allows souls; a blacklist containing one rejects them. For mod filters, enter `industrialforegoingsouls`.
