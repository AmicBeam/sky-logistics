...
- **线路连接面**：列出设备图标、抽取/存入模式、资源缩写、优先级、红石状态、坐标与维度；顺序与网络顺位一致（来源在前、接收端在后，各组优先级从高到低，同级稳定排序），右上角显示当前页码。双击当前维度方块的坐标会关闭界面、让视角转向目标，并用可透视的白色立方体高亮该方块 5 秒；支配杖覆盖同时启用时，目标方块在这 5 秒内优先显示白色，到期后恢复线路颜色。
...
从 1.3.1 起，点击配置器左侧的 **权限** 按钮可以进入权限管理，再点 **线路** 返回。权限设置属于当前玩家，统一作用于其全部线路；切换或新建线路无需重新授权。

- **线路公开** 默认关闭。开启后，所有玩家都可以复制和修改你的线路配置；关闭后，仅自己和明确授权的玩家可以操作。
- **搜索玩家** 支持按名字搜索服务器记录过的玩家（包括离线玩家），点击 **添加** 授权。授权绑定 UUID，改名不会失效。
- 清空搜索框后，列表分页展示全部明确授权的玩家；点击 **移除** 撤销权限。公开状态下的所有陌生玩家不会逐一列入授权名单。
- 服务端会校验复制、粘贴、配置修改及副手配置器绑定。撤销授权后，旧配置器和已打开的节点界面也不能继续修改私有线路。已有线路的物流传输不会因撤销权限自动拆除。
...
2. **资源**可多选物品、流体和能量。
3. **模式**选择无、抽取或存入。
4. 两个**升级插槽**可安装槽位并行升级、维度升级、强制抽取升级或顺序匹配升级。
5. **更多**进入该面的红石、维持数、优先级和过滤设置。

方向下方会显示当前相邻目标名称。未连接的面不会参与线路传输。

顺序匹配升级的逐槽模式可装在抽取端或存入端；逐个模式仅对抽取端生效，装在存入端时回退为普通存入。手持右击空气切换逐槽/逐个，默认逐槽；切到逐个会清除顺位偏移，再切回逐槽时为 0。逐槽使用“本地槽位 + 顺位偏移 = 网络顺位”：潜行上滚增大、下滚减小，正数跳过网络列表前端，负数跳过本地前端槽位，滚动不会切换快捷栏；抽取端映射接收端，存入端反向映射来源端与本地槽位。逐个模式从当前游标开始统计能接收该物品的目标，再只在这些目标间批量分配；统计和传输都分别占用预算，并可跨 tick 继续。同优先级节点不会合并。执行时若目标已不能接收，会直接跳过且不记录欠目标任务，剩余物品由后续批次重新统计。
...
- **维持数**：直接输入目标值；右侧按钮在“槽”和“个”之间切换，默认为“槽”。物品的“槽”按匹配物品槽数，“个”按物品总数；流体/化学品/Souls 的“槽”按非空储罐数，FE/Mana/魔源按非空接收设备数，“个”按各资源原生数量。达到目标单元数后默认继续填满已有匹配单元，但不会新开超出目标的单元；该行为可由 `transfers.fillMaintainedItemSlots` 关闭。来源耗尽而接收端仍有维持缺口时，会按配置的维持探测间隔检查来源恢复。
...
- **维持数**：直接输入目标值；右侧按钮在“槽”和“个”之间切换，默认为“槽”。选择“槽”时按匹配物品占用的槽位数工作，并默认在达到目标槽数后继续填满已有匹配槽，但不会新开超出目标的槽；选择“个”时按匹配物品总数工作。填满行为可由服务器配置 `transfers.fillMaintainedItemSlots` 关闭。
...
- 左上角显示仪式持续时间（tick）。
- `T1` / `T2` / `T3` 表示配方所需的最低祭坛等级；更高阶祭坛也能运行低阶配方。
