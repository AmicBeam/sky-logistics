...
顺序匹配升级装在抽取节点后，会优先处理最靠前的来源物理槽位。手持升级卡右击空气可切换逐槽/逐个，默认逐槽。逐槽按来源槽位位置映射存入端；逐个按每个抽取面持久保存的游标轮询存入端，来源数量较大时会批量分给多个端点。每个接收端都是独立位置，同优先级节点不会合并；批量访问多个端点时也会分别占用预算。逐个模式的失败分配会进入持久化扣押队列并优先重试，队列满后停止越过新失败目标。
