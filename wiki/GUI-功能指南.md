...
- **线路连接面**：列出设备图标、抽取/存入模式、资源缩写、优先级、红石状态、坐标与维度；右上角显示当前页码。
...
2. **资源**可多选物品、流体和能量。
3. **模式**选择无、抽取或存入。
4. 两个**升级插槽**可安装速度升级、维度升级或精确数量升级。
5. **更多**进入该面的红石、槽数、优先级和过滤设置。
...
- **红石**：循环选择忽略红石或其它红石条件。
- **槽数**：按过滤规则限制匹配物品占用的槽位数；`不限制` 表示不启用槽位限制。
- **数量**：安装精确数量升级时替代槽数按钮，直接输入 `1` 到 `2147483647`。抽取面保留该数量，存入面补足到该数量；统计对象是过滤规则匹配到的物品总数。
...
- **维持**：以玩家主物品栏中的匹配库存为目标，数量不足时从线路补入，超出时把多余物品发送到线路；模式文字显示为紫色。
- **槽数**：设置参与控制的匹配槽位数量；维持模式的目标需要大于 0。按住 Shift 点击 `−` / `+` 可每次调整 10。
- **优先级**：决定项链作为线路端点时的顺序。
- **升级插槽**：两个槽位可安装维度升级和精确数量升级。每种升级可安装一张。
- **维度升级**：让项链访问其它已加载维度中的同线路端点。
- **精确数量升级**：隐藏槽数调整按钮并显示数量输入框，范围为 `1` 到 `2147483647`；抽取、存入和维持模式均按白名单匹配物品的总数执行。
...
从大型外部网络抽取物品时必须配置至少包含一个物品项的白名单，避免无界扫描整个网络。
