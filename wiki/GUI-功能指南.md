...
项链只搬运物品，并且只在同维度使用白名单：

- **过滤槽**：必须放入至少包含一个物品项的白名单过滤列表；黑名单不会工作。
- **抽取**：从线路目标取出匹配物品并放入玩家或兼容背包。
- **存入**：从玩家或兼容背包取出匹配物品并发送到线路。
- **槽数**：限制参与存入模式的背包槽数量；按住 Shift 点击 `−` / `+` 可每次调整 10。
- **优先级**：决定项链作为线路端点时的顺序。
...
上方 18 个幽灵槽可保存精确物品或流体样本。它们只保存规则，不真实吞掉物品：
...
右侧红色 `×` 清空全部幽灵槽。可以从玩家物品栏点击样本，也可以从 JEI 拖入物品或流体 ghost；水桶从玩家物品栏点击时仍按物品记录，需要流体规则时请拖入 JEI 流体。
