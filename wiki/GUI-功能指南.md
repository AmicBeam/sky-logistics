...
- **维持数**：直接输入目标值；右侧按钮在“槽”和“个”之间切换，默认为“槽”。“槽”按匹配物品占用的槽位数工作，并默认在达到目标槽数后继续填满已有匹配槽，但不会新开超出目标的槽；“个”按匹配物品总数工作。每个方向独立保存设置，填满行为可由服务器配置 `transfers.fillMaintainedItemSlots` 关闭。
