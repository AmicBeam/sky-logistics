...
- **维持数**：直接输入目标值；右侧按钮在“槽”和“个”之间切换，默认为“槽”。过滤规则先确定匹配集合，“槽”统计集合合计占用的槽位，“个”统计集合中的物品总数；多种匹配物品共用一个合计目标，不会逐种分别维持。按槽模式默认在达到目标槽数后继续填满已有匹配槽，但不会新开超出目标的槽。每个方向独立保存设置，填满行为可由服务器配置 `transfers.fillMaintainedItemSlots` 关闭。
...
- **维持数**：直接输入目标值；右侧按钮在“槽”和“个”之间切换，默认为“槽”。过滤白名单先确定匹配集合，“槽”或“个”对集合做合计，多种白名单物品不会逐种分别维持。按槽模式默认在达到目标槽数后继续填满已有匹配槽，但不会新开超出目标的槽；填满行为可由服务器配置 `transfers.fillMaintainedItemSlots` 关闭。
