...
顺序匹配升级可装在抽取端或存入端。手持右击空气切换逐槽/逐个，默认逐槽；切到逐个会清除顺位偏移，再切回逐槽时为 0。逐槽使用“本地槽位 + 顺位偏移 = 网络顺位”：潜行上滚增大、下滚减小，正数跳过网络列表前端，负数跳过本地前端槽位，滚动不会切换快捷栏；抽取端映射接收端，存入端反向映射来源端与本地槽位。逐个则分别轮询接收端或接收库存槽，成功后推进；来源数量较大时会批量分给多个端点，每个端点分别占用预算。同优先级节点不会合并。逐个抽取失败会进入持久化扣押队列并优先重试，队列满后停止越过新失败目标。
