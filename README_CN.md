...
- **多容器一键连接**：天穹分配器允许一个物流节点或简易管道代理一组相连的兼容容器。兼容扳手可按 ME 样板供应器相同的交互和六面显示方式，在全向与六个定向绑定状态间切换；定向扫描只从所选方向开始，并可穿过其他分配器，但每个被穿过的分配器会占用 1 个目标名额。输入默认会在可接收目标间尽量均分；接收到红石信号时则改为按游标顺序依次存放。抽取会聚合所有目标；默认最多代理 32 个目标，可配置至 64 个。
