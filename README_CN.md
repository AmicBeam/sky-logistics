...
- **高吞吐量**：普通节点每 tick 扫描 1 个槽位，每张堆叠的槽位并行升级额外增加 1 个；默认一个升级格最多放 8 张，可达到 9 槽/t。标准物品与 FE 操作采用可配置的 2.1G 级上限，两端均支持 long 数量时，直连单次操作上限可配置至约 9.22e18。简易管道每个抽取端默认每 tick 可搬运 64 个物品、10,000 mB 流体、100,000 FE、10,000 化学品、50 mana 或 50 Source。
- **多容器一键连接**：天穹分配器允许一个物流节点或简易管道代理一组相连的兼容容器。兼容扳手可按 ME 样板供应器相同的交互和六面显示方式，在全向与六个定向绑定状态间切换；定向扫描的整条探索链都只沿所选方向直线推进，并可穿过同一直线上的其他分配器，但每个被穿过的分配器会占用 1 个目标名额。切换方向会立即废弃旧绑定路径并重新发现。输入默认会在可接收目标间尽量均分；接收到红石信号时则改为按游标顺序依次存放。抽取会聚合所有目标；默认最多代理 32 个目标，可配置至 64 个。
...
3. 默认在 Y 96 或更高处，或配置的供奉高度为尤洛伽水晶充能，并用它制作天穹石和高级天穹物流组件。
4. 搭建带供桌的分阶天穹供奉祭坛：二阶可制作柯拉甘露，再用 4 个压缩甘露块升级为工作倍率可配置的三阶祭坛。
5. 放置天穹物品库或天穹流体库，作为聚合仓储端点。
6. 将天穹物流节点放在机器、仓库、分配器或外部存储接口旁；普通放置为存入模式，潜行放置为抽取模式。
7. 使用天穹配置器创建和管理命名线路、复制粘贴节点设置，并通过副手为新放置节点预设配置。
8. 按需加入过滤列表、标签过滤列表、槽位并行升级和维度升级。
...
- 仓库类型上限、节点物品/能量单次传输量、天穹容器间直连传输量、分配器目标数与 BFS 扫描预算、热槽缓存大小、供奉高度、三阶祭坛工作倍率、配石充能高度和充能时间均可配置；分配器传输探测使用线路操作预算。
...
仅使用 GNU 通用公共许可证第 3 版（`GPL-3.0-only`）。完整文本见根目录 `LICENSE`；各版本目录也保留一份副本，随构建产物分发。
