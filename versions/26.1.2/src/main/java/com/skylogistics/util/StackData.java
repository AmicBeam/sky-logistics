...
public final class StackData {
    ...
    public static CompoundTag saveItem(ItemStack stack, HolderLookup.Provider registries) {
        ...
        Tag saved = ItemStack.OPTIONAL_CODEC.encodeStart(registries.createSerializationContext(NbtOps.INSTANCE), stack)
    ...
    public static ItemStack loadItem(CompoundTag tag, HolderLookup.Provider registries) {
        return tag == null || tag.isEmpty() ? ItemStack.EMPTY
                : ItemStack.OPTIONAL_CODEC.parse(registries.createSerializationContext(NbtOps.INSTANCE), tag)
    ...
    public static CompoundTag saveFluid(FluidStack stack, HolderLookup.Provider registries) {
        ...
        Tag saved = FluidStack.OPTIONAL_CODEC.encodeStart(registries.createSerializationContext(NbtOps.INSTANCE), stack)
                .result().orElse(new CompoundTag());
        return saved instanceof CompoundTag compound ? compound : new CompoundTag();
    }
    ...
    public static FluidStack loadFluid(CompoundTag tag, HolderLookup.Provider registries) {
        return tag == null || tag.isEmpty() ? FluidStack.EMPTY
                : FluidStack.OPTIONAL_CODEC.parse(registries.createSerializationContext(NbtOps.INSTANCE), tag)
