...
public class ConfiguratorScreen extends AbstractContainerScreen<ConfiguratorMenu> {
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        if (permissionMode) {
            graphics.text(font, Component.translatable("screen.skylogistics.permissions.title"), 10, 7, ConfigPanel.TEXT, false);
            graphics.text(font, Component.translatable("screen.skylogistics.permissions.search"), 12, 45, ConfigPanel.MUTED, false);
            var snapshot = menu.permissions();
            ...
            graphics.text(font, label, 12, 77, ConfigPanel.MUTED, false);
    ...
    private void drawBottomFieldset(GuiGraphicsExtractor graphics, int x, int width, Component legend) {
        ConfigPanel.drawFieldset(graphics, leftPos + x, topPos + BOTTOM_GROUP_Y, width, font.width(legend));
    }

    private void drawBottomValueBoxes(GuiGraphicsExtractor graphics) {
    ...
    private Identifier guiTexture(String name) {
        return Identifier.fromNamespaceAndPath("skylogistics", CONFIGURATOR_TEXTURE_ROOT + name);
    }
    ...
    private final class PermissionButton extends AbstractButton {
        ...
        @Override public void onPress(net.minecraft.client.input.InputWithModifiers input) { if (active) pressed.run(); }
