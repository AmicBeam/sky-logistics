...
public class FilterListScreen extends AbstractContainerScreen<FilterListMenu> {
    ...
    public FilterListScreen(FilterListMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title, 224, 191);
        inventoryLabelY = 97;
    }
    ...
    protected void extractTooltip(GuiGraphicsExtractor graphics, int x, int y) {
        ...
        if (slot >= 0 && menu.isFluidFilter(slot)) {
            ...
            if (!fluid.isEmpty()) {
                graphics.setTooltipForNextFrame(font, fluid.getHoverName(), x, y);
                return;
            }
        }
    ...
    private boolean renderButtonTooltip(GuiGraphicsExtractor graphics, int x, int y) {
        ...
        if (nbtIgnoreButton != null && nbtIgnoreButton.isMouseOver(x, y)) {
            graphics.setTooltipForNextFrame(font, Component.translatable("tooltip.skylogistics.filter_list.nbt_ignore_button"),
                    x, y);
            return true;
        }
        if (nbtMatchButton != null && nbtMatchButton.isMouseOver(x, y)) {
            graphics.setTooltipForNextFrame(font, Component.translatable("tooltip.skylogistics.filter_list.nbt_match_button"),
                    x, y);
            return true;
        }
    ...
    private static void drawSegmentButton(GuiGraphicsExtractor graphics, AbstractButton button, boolean selected, int color) {
        if (button == null) {
            return;
        }
    ...
    private static void drawNbtIcon(GuiGraphicsExtractor graphics, AbstractButton button, boolean enabled, boolean selected) {
        if (button == null) {
            return;
        }
        int color = ConfigPanel.buttonTextColor(button.active);
    ...
    private static void drawDurabilityIcon(GuiGraphicsExtractor graphics, AbstractButton button, boolean enabled, boolean selected) {
        if (button == null) {
            return;
        }
        int color = ConfigPanel.buttonTextColor(button.active);
