...
- 物品、流体、能量共用同一套线路 ID、权限和操作率配置。
- 不引入 Flux 式网络能量池；本模组保持无中间缓存，能量也必须在同一事务里完成源端抽取和目标接收。
...
- 传输速度不以“物品数/t”为主要语言，而以“堆叠/t、tank/t 或端点操作/t”为主要限制。
...
- `已充能天穹水晶`：同一物品通过 damage 状态区分图标；在配置的高空仪式高度或更高处的玩家背包中，或放在本模组 `天穹供桌` 中一段时间后完成充能。默认高度为 `Y >= 96`，可通过服务端配置 `rituals.skyRitualMinY` 调整。
...
三阶保留二阶外角柱，并把内侧 5x5 外圈的四个角替换为柯拉甘露块。三阶按 `rituals.tierThreeAltarWorkSpeedMultiplier` 推进工作进度，默认 4 倍；粒子效果使用独立时间轴，不随工作倍率加速。
...
- 祭坛只在配置的高空仪式高度或更高处，且多方块结构有效时工作。默认高度为 `Y >= 96`，可通过服务端配置 `rituals.skyRitualMinY` 调整。
- 一阶结构使用祭坛下方一层的完整 5x5 天穹石外圈，结构不要求其它位置为空气；二阶保留一阶结构并在 7x7 外扩四角增加天穹石柱与天穹玻璃柱顶；三阶保留二阶外柱，但把内侧 5x5 框架四角替换为柯拉甘露块。
...
- Patchouli 数据提供一本 `天穹物流手册`，并分别提供一阶、二阶和三阶多方块预览。JEI 页面作为可选 compat 源码存在，默认无 JEI API jar 时不参与基础构建。
...
| `types` | 当前面允许物品、流体、能量；资源类型是面级配置，同一节点不同面可以启用不同资源 |
| `mode` | 输入源、输出目标、双向、禁用 |
| `side` | 指定机器侧面或全部侧面 |
...
| `tickDelay` | 基础间隔 |
| `redstone` | 当前面的红石条件：忽略、高信号、低信号、上升沿脉冲 |
...
- 当前面的资源类型可多选：物品、流体、能量。资源开关按面保存，不同面可以分别负责物品、流体或能量。
- 侧面可选：当前面、指定方向或全部侧面。
- “更多”按钮切换当前面的高级配置页。
- 每个面可配置 `priority` 数值；目标选择固定为 priority 分组，高 priority 先尝试，同 priority 组内轮询。
- 每个面可配置 `redstone`：忽略、高信号、低信号或上升沿脉冲；脉冲成功搬运一次后休眠，被红石条件挡住的面不计入失败冷却。
...
- 当前侧面是否存在 item/fluid/energy capability。
- 是否可能抽取物品、流体或能量。
...
```text
候选源 = select up to operationRate stacks/tanks/keys from source
...
本模组的默认限速语言不是“物品数/t”，而是“堆叠/t”或“操作/t”。

对物品来说：

- `1 堆叠/t` 表示每 tick 最多处理 1 个源槽或 1 个聚合资源 key。
- 原版箱子的一格普通物品最多 64 个，因此箱子到箱子的默认表现会自然接近 `64/t`。
- 本模组物品天仓没有传统槽位，同一种物品类型视作一个“虚拟格”。天仓到天仓传输圆石时，`1 堆叠/t` 可以在一次事务里移动非常大的 `long amount`。
...
- Forge 1.20.1 的很多流体和 energy capability 使用 `int` 参数，单次调用理论上最多约 `2.1B`，实际还会被目标容量和实现 clamp。
...
| 原版箱子到原版箱子，`1 堆叠/t` | 每 tick 固定少量 slot/capability 调用 |
| exact/NBT/组件过滤混合 | 过滤耗时随条目数次线性或接近常数，不能每 tick 解析 |
| 640000 物品请求 | 单 tick 目标尝试数受硬预算限制 |

结论：我们最应该超过 LogisticsNetworks 的地方，是“空闲网络”“目标满网络”“过滤重网络”和“本模组聚合容器互传”。对任意第三方机器的大规模 I/O，LogisticsNetworks 已经做了不少正确优化，我们的目标是靠更少功能面、更强计划编译和堆叠/t 限速取得更低常数成本。
...
本模组不引入 Flux 式网络能量池，因此能量采用“直接事务 + 短退避”：
...
- 能量输入源只读取 `energyOutputs`，不遍历物品/流体输出。
- 输出按优先级排序，同优先级内用游标轮询，避免固定第一个目标吃满所有能量。
- 每个目标尝试前先检查目标红石、资源开关、短 TTL 失败缓存，再调用 `receiveEnergy(..., true)` 做需求模拟。
- 如果线路有跨维升级，使用全局 `lineId -> energyOutputs` 缓存；否则只使用当前维度的能量输出列表。

能量退避结论：

- 暂不实现能量专用短退避。`1t -> 2t -> 5t` 这类方案能降低恢复延迟，但收益点不明确，会让空源、满电池和离线目标在大型线路里更频繁地 simulate 外部 energy capability。
- 当前维持通用失败退避和短 TTL 目标拒收缓存；预算耗尽不是失败，线路下一 tick 继续排队，不额外增加能量退避。
- 只有在真实压测或玩家反馈证明能量供电抖动明显时，再考虑热端点专用策略，例如只对刚成功过的能量源/目标缩短恢复等待。

这个取舍更适合大型整合包常加载工业区：资源索引先减少无关目标扫描，能量延迟策略保守保留，避免为了不明确的体感收益制造常驻扫描成本。
...
- `已充能天穹水晶`：在配置的高空仪式高度或更高处的玩家背包或天穹供桌中等待充能完成；用于合成天穹石和祭坛。默认高度为 `Y >= 96`，可通过服务端配置 `rituals.skyRitualMinY` 调整。
...
- `空域配置器`、`天穹过滤列表`、槽位并行升级：进入常规工作台合成路线，部分升级消耗星辉甘露；跨维升级通过二阶天穹供奉获得。

推荐进程：

1. 前期：工作台合成天穹水晶 -> 高空充能 -> 合成天穹石 -> 搭建祭坛/供桌法阵。
2. 中期：通过天穹供奉获得星辉甘露 -> 合成物品天仓/流体天池和基础节点。
3. 后期：配置工具设置线路 ID -> 过滤升级 -> 能量传输 -> 跨维升级。
...
- 空域节点。
- 物品、流体、能量三类资源传输。
- 线路基础字段：启用、发送/接收、面级资源类型、侧面、每面 2 个过滤槽、堆叠/t 操作率、延迟、优先级。
- 空域配置器。
- 空域过滤列表物品。
...
- 星辉甘露供奉配方。
- 祭坛/供桌/天穹石/天穹玻璃/柯拉甘露块组成的供奉多方块结构，支持一阶框架、二阶外扩角柱和三阶甘露角判断。
- Patchouli 手册数据和一阶/二阶/三阶多方块预览条目。
- 复制粘贴配置。
- 失败 backoff。
- 存档重载后的普通端点重绑。
...
| Flux Networks | 无线能量网络 | 扩展为物品/流体/能量的线路模型，气体留到后期联动 |
