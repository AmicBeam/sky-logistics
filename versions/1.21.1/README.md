...
- `Simple Celestial Pipes and Sky Wrench` / `天穹简易管道与天穹扳手`
  ...
  - Chemical, mana and Source limits have independent server settings. `simplePipeMaxConnectedBlocks` defaults to 1024; a connection that would exceed it stays disconnected unless `enforceSimplePipeConnectionLimit` is disabled.
