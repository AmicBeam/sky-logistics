...
- `已充能天穹水晶`：同一物品通过 damage 状态区分图标；在配置的高空仪式高度或更高处的玩家背包中，或放在本模组 `天穹供桌` 中一段时间后完成充能。默认高度为 `Y >= 128`，可通过服务端配置 `rituals.skyRitualMinY` 调整。
...
- 祭坛只在配置的高空仪式高度或更高处，且多方块结构有效时工作。默认高度为 `Y >= 128`，可通过服务端配置 `rituals.skyRitualMinY` 调整。
...
- `已充能天穹水晶`：在配置的高空仪式高度或更高处的玩家背包或天穹供桌中等待充能完成；用于合成天穹石和祭坛。默认高度为 `Y >= 128`，可通过服务端配置 `rituals.skyRitualMinY` 调整。
