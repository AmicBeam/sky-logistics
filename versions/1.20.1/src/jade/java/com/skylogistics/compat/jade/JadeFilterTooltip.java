...
final class JadeFilterTooltip {
    ...
    static CompoundTag write(NetworkEndpointBlockEntity endpoint) {
        CompoundTag result = new CompoundTag();
        for (Direction direction : Direction.values()) {
            ItemStack filter = endpoint.getFaceFilter(direction, 0);
            if (!filter.isEmpty()) result.put(direction.getSerializedName(), filter.save(new CompoundTag()));
        }
        return result;
    }
    static void append(ITooltip tooltip, CompoundTag filters, Direction direction, Player player) {
        ...
        ItemStack filter = ItemStack.of(filters.getCompound(direction.getSerializedName()));
