...
public final class SimplePipeJadeProvider extends BaseSkyLogisticsJadeProvider
    ...
    @Override public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        JadeTransferRateTooltip.append(tooltip, pipeData.getCompound("ProgressionRates"));
        JadeFilterTooltip.append(tooltip, pipeData, direction, accessor.getPlayer());
    }
    @Override public void appendServerData(CompoundTag data, BlockAccessor accessor) {
        if (accessor.getBlockEntity() instanceof SimplePipeBlockEntity pipe) data.put(DATA, writePipeData(pipe));
    }
    private static CompoundTag writePipeData(SimplePipeBlockEntity pipe) {
        CompoundTag data = JadeFilterTooltip.write(pipe);
        int activeFaces = 0;
