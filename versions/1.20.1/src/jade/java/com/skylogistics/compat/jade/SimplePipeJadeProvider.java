...
public final class SimplePipeJadeProvider extends BaseSkyLogisticsJadeProvider
    ...
    @Override public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        JadeFilterTooltip.append(tooltip, accessor.getServerData().getCompound(DATA), direction, accessor.getPlayer());
    }
    @Override public void appendServerData(CompoundTag data, BlockAccessor accessor) {
        if (accessor.getBlockEntity() instanceof SimplePipeBlockEntity pipe) data.put(DATA, JadeFilterTooltip.write(pipe));
    }
}
