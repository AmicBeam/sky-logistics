...
- `Simple Celestial Pipes and Sky Wrench` / `天穹简易管道与天穹扳手`
  ...
  - Simple pipe settings are grouped under `transfers.simplePipes`. Chemical, mana and Source limits are independent; recipes use diamonds; `simplePipeMaxConnectedBlocks` defaults to 1024, and `enforceSimplePipeConnectionLimit` can disable the connection check.
